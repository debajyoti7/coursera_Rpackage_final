---
title: "Overview of the farsDebajyoti package"
author: "Debajyoti Nag"
date: "`r Sys.Date()`"
...
Further we give a short description with examples how to use the functions. For the purposes of these examples we will use data from FARS, years 2013-2015, that can be found in the package directory under `data` folder.
...
```{r fars_read_example}
print(getwd())
filename <- system.file("extdata/accident_2013.csv.bz2", package = "farsDebajyoti")
fars_read(filename)
```
...
The last function available in the package, `fars_map_state()` takes a state index and year as arguments and returns a plot of the state with accident locations. The state indices need to correspond available indices from the data set. Here are some examples:
